...
public sealed class TitleBgmPlayer : MonoBehaviour
{
    [Header("Audio")]
    ...
    [SerializeField] private bool _fadeOutOnDisable = false;
    ...
    public void Play()
    {
        ...
        if (_fadeIn)
        {
            FadeTo(_volume, _fadeInDuration, stopAfterFade: false);
        }
        else
        {
    ...
    private void FadeTo(float targetVolume, float duration, bool stopAfterFade)
    {
        StopFadeCoroutine();
        _fadeCoroutine = StartCoroutine(FadeRoutine(Clamp01(targetVolume), duration, stopAfterFade));
    }
